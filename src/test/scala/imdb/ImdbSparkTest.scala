...
class ImdbSparkTest extends AnyFlatSpec with Matchers with BeforeAndAfterAll {
  ...
  private def printTestStatus(taskName: String, executionTime: Long, timeLimit: Long, passed: Boolean): Unit = {
    val statusSymbol = if (passed) "✓" else "✗"
    val timeStatus = if (executionTime <= timeLimit) "✓" else "✗"
    println(s"""
               |----------------------------------------
               |Test Results for $taskName:
               |Status: $statusSymbol ${if (passed) "PASSED" else "FAILED"}
               |Execution Time: ${executionTime}ms ${timeStatus} (Limit: ${timeLimit}ms)
               |----------------------------------------
               |""".stripMargin)
  }
  ...
  "Task1: Genre Growth Analysis" should "find the correct top 5 genres with growth" in {
    ...
    val timeWithinLimit = executionTime <= 10000L

    val passed = correctLength && correctContent && timeWithinLimit
    printTestStatus("Task 1", executionTime, 1000L, passed)
    ...
    if (!timeWithinLimit) println(s"Time limit exceeded: ${executionTime}ms > 1000ms")

    result should have length 5
    result should contain theSameElementsInOrderAs expected
    executionTime should be <= 1000L
  }
  ...
  "Task2: Genre Ratings by Decade" should "calculate correct weighted averages per decade" in {
    ...
    val passed = allMatches && executionTime <= 3000L && result.length == expected.length
    printTestStatus("Task 2", executionTime, 3000L, passed)
    ...
    if (executionTime > 3000L) println(s"Time limit exceeded: ${executionTime}ms > 3000ms")
  ...
  "Task3: Top Directors" should "find directors with highest rated films" in {
    ...
    val startTime = System.currentTimeMillis()
    val result = ImdbSpark.task3(titleBasicsRDD, titleRatingsRDD, titleCrewRDD).collect().toList
    val endTime = System.currentTimeMillis()
    val executionTime = endTime - startTime
    ...
    val passed = correctLength && correctContent && executionTime <= 1500L
    printTestStatus("Task 3", executionTime, 1500L, passed)
    ...
    if (executionTime > 1500L) println(s"Time limit exceeded: ${executionTime}ms > 1500ms")
  ...
  "Task4: Prolific Crew Members" should "identify top contributors across time periods" in {
    val expected = List(
      ("Marco Romano", 12),
      ("Jordan Hill", 12),
      ("Sergey A.", 10),
    ...
    val correctContent = result == expected
    val timeWithinLimit = executionTime <= 2000L

    val passed = correctLength && correctContent && timeWithinLimit
    printTestStatus("Task 4", executionTime, 2000L, passed)
    ...
    if (!timeWithinLimit) println(s"Time limit exceeded: ${executionTime}ms > 2000ms")
...
// SparkContext wrapper for singleton pattern
